...
- **THEN** 浏览器自动回退到 system-ui 字体，无 FOIT（Flash of Invisible Text）
...
- **WHEN** 页面首次加载
- **THEN** next/font 自动下载并注入 Noto Sans SC 字体资源（400/500/600/700 四个字重）

#### Scenario: 字体声明正确的 CSS 变量

- **WHEN** 字体加载配置完成
- **THEN** `--font-inter` 和 `--font-noto-sans-sc` 变量正确注入到 HTML 元素
