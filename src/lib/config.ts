...
interface ModelsConfigFile {
    ...
    // Support new mcp object format keyed by server name
...
export async function getBuiltinMcpConfigs(): Promise<BuiltinMcpServer[]> {
    ...
    // Handle new format: mcp object keyed by server name
    if (config.mcp && !config.builtinMcpServers) {
        const mcpObj = config.mcp;
        return Object.entries(mcpObj).map(([name, server]) => ({
            name,
